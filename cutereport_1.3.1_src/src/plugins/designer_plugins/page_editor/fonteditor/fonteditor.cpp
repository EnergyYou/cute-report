  ...
  ,m_doProcessControls(true)
{
    ...
    connect(ui->family, SIGNAL(currentIndexChanged(int)), this, SLOT(fontFamilyToObjectProperty()));
    connect(ui->size, SIGNAL(valueChanged(double)), this, SLOT(fontSizeToObjectProperty()));
...
void FontEditor::fontFamilyToObjectProperty()
{
    ...
    if (!objectList.isEmpty()) {
        foreach (QObject * object, objectList) {
            ...
            f.setFamily(ui->family->currentText());
            object->setProperty(m_fontPropertyName.toLatin1(), f);
        }
    }
}
